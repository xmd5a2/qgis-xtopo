...
def main():
    ...
    layout = [
        [sg.Text('QGIS-xtopo', font='Any 20', text_color="#E9FFEA")] +
        [sg.Column([[]], size=(506, 50), pad=(0, 0))] +
        [sg.Column(
            [
                [sg.Button('RU', key="button_ru", button_color=('white', '#497E90'), visible=True)] +
                [sg.Button('EN', key="button_en", button_color=('white', '#497E90'), visible=False)]
            ], size=(60, 40))
        ],
        ...
        ,
        [sg.Frame(layout=[
            [sg.Text(translations.get('overpass_instance', 'Overpass instance'), key='overpass_instance_text',
                     size=(22, 1),
                     justification='l',
                     tooltip=translations.get('overpass_instance_tooltip',
                                              'Select the type of server for receiving OSM data'))] +
            [sg.Radio('docker', "RADIO1", key=r_keys[0], change_submits=True, default=True, size=(5, 1)),
             sg.Radio('external', "RADIO1", key=r_keys[1], change_submits=True)],
            [sg.Text(translations.get('osm_data_files', 'OSM data file(s)'), key='osm_data_files_text', size=(21, 1),
                     justification='l')] +
            [sg.Input(key='osm_files', disabled=True,
                      tooltip=translations.get('osm_data_files_tooltip', 'OSM data files list'), size=(60, 1)),
             sg.FilesBrowse(
        ...
        [sg.Frame(layout=[
            [sg.Text(translations.get('calc_tiles_list', 'List of required terrain tiles'), key="calc_tiles_list_text",
                     size=(21, 1), justification='l', text_color='yellow'),
             sg.Input(key='calc_tiles_list',
                      size=(60, 1), readonly=True)] +
            [sg.Text(translations.get('total', 'Total') + ": ", key="total_text",
                     size=(5, 1), justification='l'),
             ] +
            [sg.Text('', key="total_tiles_value",
            ...
            [sg.Text(translations.get('use_terrain_src_dir', 'Use terrain source directory'),
             ...
             sg.Radio('', "RADIO2", key=terrain_radio_keys[0], change_submits=True, default=True, size=(1, 1)),
    ...
    window = sg.Window('QGIS-xtopo-GUI', layout, finalize=True, icon=logo_icon)
    ...
    while True:
        ...
        if event == 'button_en':
            ...
            window.Elem('button_en').update(visible=False)
        if event == 'bbox':
            update_layout_calc_tiles_list(values['bbox'], translations, window)
        if event == 'Paste bbox':
            clipboard_contents = pyperclip.paste()
            window.Elem('bbox').update(clipboard_contents)
            update_layout_calc_tiles_list(clipboard_contents, translations, window)
        if event == 'Clear':
        ...
        if event == r_keys[0]:
            ...
            window.Elem('osm_data_files_text').update(text_color="white")
            window.Elem('populate_db').update(disabled=False)
        if event == r_keys[1]:
            window.Elem('select_osm_files').update(disabled=True)
            window.Elem('osm_data_files_text').update(text_color="#B6B4C3")
            window.Elem('populate_db').update(disabled=True)
        ...
        if event == 'populate_db':
            if check_parameters(values, translations, osm_data_dir) is False:
                continue
            start(docker_installed, values, window, False, translations)
            ...
            if os.name == "posix":
                terminal_command_params_list = get_terminal_command_params_list(
                    'docker exec -it --user user qgis-xtopo /app/populate_db.sh', '', True, translations)
        ...
        if event == 'prepare_data':
            ...
            if os.name == "posix":
                terminal_command_params_list = get_terminal_command_params_list(
                    'docker exec -it --user user qgis-xtopo /app/prepare_data.sh', '', True, translations)
                subprocess.Popen(terminal_command_params_list).wait()
            else:
                if os.name == "nt":
                    subprocess.Popen(
        ...
        if event == 'open_qgis':
            start(docker_installed, values, window, False, translations)
            run_qgis(window, translations)
        if event == 'start':
            if check_parameters(values, translations, osm_data_dir) is False:
                continue
            start(docker_installed, values, window, values["run_chain"], translations)
...
def update_layout_calc_tiles_list(bbox, translations, window):
    ...
    if srtm_tiles_list_size >= 8:
        ...
        if srtm_tiles_list_size > 4 and srtm_tiles_list_size < 8:
            window.Elem('total_tiles_value').update(text_color='orange')
            window.Elem('total_text').update(text_color='orange')
        else:
            window.Elem('total_tiles_value').update(text_color='white')
            window.Elem('total_text').update(text_color='white')
...
def run_qgis(window, translations):
    if os.name == "posix":
        if is_tool("xhost"):
...
def start(docker_installed, values, window, run_chain, translations):
    if docker_installed:
    ...
    if values[r_keys[0]] and len(osm_files_list) > 1:
        print("Copying files" + " " + str(osm_files_list))
    ...
    if len(srtm_tiles_list.split(" ")) > 10:
        sg.Popup(translations.get('bounding_box_too_large', 'Bounding box is too large. Creating a map will take a '
                                                            'long time'), translations.get('warning', 'Warning'))
    if docker_installed:
        if "qgis-xtopo" in str(
                subprocess.check_output(['docker', 'ps'], stdin=subprocess.PIPE, stderr=subprocess.STDOUT)):
            if os.name == "posix":
                ...
                terminal_command_params_list = get_terminal_command_params_list(
                    "docker exec --user user qgis-xtopo /app/init_docker.sh", run_chain_filename, False, translations)
                subprocess.Popen(terminal_command_params_list).wait()
                try:
                    os.remove(run_chain_filename)
                except OSError:
...
def get_terminal_command_params_list(cmd, run_chain_filename, hold, translations):
    get_terminal_name()
...
def read_config_update_ui(window, values, translations):
    ...
    if os.path.isfile(config):
        ...
        if overpass_instance_config == "external":
            ...
            window.Elem('select_osm_files').update(disabled=True)
            window.Elem('osm_data_files_text').update(text_color="#B6B4C3")
        if get_setting(config, "generate_terrain") == "true":
            window.Elem('generate_terrain').update(value=True)
            switch_layout_terrain(True, window)
            if get_setting(config, "download_terrain_tiles") == "false" and get_setting(config,
                                                                                        "get_terrain_tiles") == "false":
                window.Elem('download_terrain_tiles_manually').update(value=True)
        else:
            window.Elem('generate_terrain').update(value=False)
            switch_layout_terrain(False, window)
        if get_setting(config, "get_terrain_tiles") == "true":
            window.Elem('get_terrain_tiles').update(value=True)
            window.Elem('terrain_src_dir').update(disabled=False)
            window.Elem('terrain_src_dir_browse').update(disabled=False)
        else:
            window.Elem('get_terrain_tiles').update(value=False)
        if get_setting(config, "download_terrain_tiles") == "true":
...
def update_layout_translations(translations, window):
    ...
    window.Elem('osm_data_files_text').update(translations.get('osm_data_files', 'OSM data file(s)'))
...
def switch_layout_terrain(generate_terrain, window):
    if generate_terrain:
        window.Elem('use_terrain_src_dir_text').update(text_color="white")
        window.Elem('terrain_src_dir').update(disabled=True)
        window.Elem('terrain_src_dir_browse').update(disabled=True)
...
def check_parameters(values, translations, osm_data_dir):
    ...
    if values[r_keys[0]] and len(osm_files_list) < 2 and not check_osm_data_is_present(osm_data_dir):
