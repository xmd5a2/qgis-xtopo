...
#### 2. Подготовка к печати
   ...
   1. **Настройка макета**
      ...
      ![print_composer_tweak_scalebar](/docs/print_composer_tweak_scalebar.png)
        ...
        Чтобы настроить масштабную шкалу (находится слева внизу карты) ![scalebar](/docs/scalebar.png) выберите **<Scalebar> - Свойства элемента - Сегменты - Фиксированная ширина**. Отредактируйте длину сегмента так, чтобы шкала была читаема и удобна для восприятия. Длина сегмента задаётся в метрах. Обычно используются значения 50, 100, 1000, 10000 и т.п. 
   ...
   2. **Создание атласа** (не обязательно)
      ...
      Если ваша карта должна быть на нескольких листах, то будет очень полезно создать атлас в QGIS.
