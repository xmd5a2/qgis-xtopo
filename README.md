...
### 3. Подготовка данных
   ...
   2. Получить данные рельефа (опционально)
      ...
      * Использовать фрагментарные данные (к примеру, несколько фрагментов нужной области). Их нужно поместить в каталог **путь_к_каталогу_с_проектами/имя_проекта/input_terrain/**.
        ...
        * [CGIAR-CSI SRTM](http://srtm.csi.cgiar.org/srtmdata/) **(рекомендуется)**
        ...
        ```
        предварительно задав в **config.ini** параметр **bbox** (шаг [**4**](/qgis-topo#4-%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B0)).
  ...
  #### Если вы хотите использовать рельеф то не забудьте задать для опции **generate_terrain** значение **true** в **config.ini** (шаг [**4**](/qgis-topo#4-%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B0)).
