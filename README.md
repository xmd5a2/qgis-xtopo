...
#### 2. Подготовка к печати
   ...
   1. **Настройка макета**
        ...
        Вероятнее всего вы увидите белый лист. Это значит что охват окна не совпадает с охватом карты, карта где-то далеко за пределами. Чтобы это исправить выберите на панели справа **Элементы - Карта - Свойства элемента - Применить охват основного окна к элементу карты** (1-4). Чтобы изменить масштаб, введите его в поле **Масштаб** (5):
      ...
      ![print_composer_move_contents_button](/docs/print_composer_move_contents_button.png)
        ...
        Чтобы настроить масштабную шкалу (находится слева внизу карты) выберите **<Scalebar> - Свойства элемента - Сегменты - Фиксированная ширина**. Отредактируйте длину сегмента так, чтобы шкала была читаема и удобна. Длина задаётся в метрах. Обычно используются значения 50, 100, 1000, 10000 и т.п. 
   ...
   2. **Создание атласа** (не обязательно)
      ...
      Если ваша карта должна быть на нескольких листах, то будет очень полезно создать атлас в QGIS. Если кратко то план следующий:
      * Создать векторный слой типа "Полигон": **Слой - Создать слой - Создать слой Shapefile**. Выбрать имя файла и путь (кнопка с тремя точками правее). **Тип геометрии - Полигон**. **OK**.
        
        ![create_shape_file](/docs/create_shape_file.png)

      * Для удобства создания полигонов копируем стиль с примера атласа. **Слои (панель) - atlas template - контекстное меню (правая кнопка мыши) - Стили - Копировать стиль - Все категории**. Далее вставляем стиль на наш слой атласа: **Слои (панель) - имя_слоя_атласа - контекстное меню (правая кнопка мыши) - Стили - Вставить стиль - Все категории**. Слой будет полупрозрачным с подписями номеров листов.
      ...
      * Добавить в слой прямоугольники (точность не особо важна), приблизительно соответствующие границам каждого листа (по одному на лист):
        ...
        Нажимаем кнопки 1, 2, 3, рисуем прямоугольник для первого листа. Правая кнопка мыши завершает редактирование элемента. Вводим номер листа (**id**). **OK**. При необходимости меняем его размеры кнопкой **Редактировать узлы** (5).
        ...
        Нам нужно задать номер страницы нового листа. Для этого выделяем его и нажимаем кнопку **Изменить аттрибуты всех выделенных объектов** (1). Вводим номер страницы в поле **id**: 2. Границы второго листа заданы. Таким же образом создаём полигоны для остальных листов.
      ...
      * В редакторе макета сгенерировать атлас: **Атлас - Сгенерировать атлас**. Указать **ваш только что созданный слой покрытия атласа**. Выключите и включите галочку **Hidden coverage layer** (Скрыть слой покрытия).
   ...
   3. **Экспорт**
        ...
        Печать из QGIS запущенного в контейнере *docker* не реализована, поэтому возможен только экспорт в файл. Можно экспортировать в растровое (**рекомендуется**) или векторное (SVG, PDF) изображение.
