...
### 1. Установка
  ...
  2. Скачать образ проекта QGIS-topo с [DockerHub](https://hub.docker.com/repository/docker/xmd5a2/qgis-topo). Команды выполняются в командной строке (терминал):
   ```
   docker pull xmd5a2/qgis-topo:latest
   ```
  3. _**Этот шаг нужен только для удобства запуска. Его можно пропустить если вы хотите запускать docker из командной строки**_. Требуется **wget**.
...
### 4. Настройка
  Конфигурационный файл проекта называется **config.ini**. Он находится здесь: **путь_к_каталогу_с_проектами/qgistopo-config/config.ini**. В нём задаются параметры, которые используются в шаге **5**.
  
  Обычно требуется менять только переменную **bbox**. Но если вы хотите использовать рельеф (шаг **3.2**) то нужно для опции **generate_terrain** установить значение **true**.
  
   * **project_name**: имя каталога проекта. Не должно содержать пробелов. Задаётся в шаге **2** при запуске *docker* контейнера. Менять не требуется.
