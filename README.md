...
### 2. Инициализация
   ...
   * С помощью скрипта `docker_run` в каталоге **qgis-topo**, скачанном на шаге **[1.2](https://github.com/xmd5a2/qgis-topo#1-%D1%83%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%BA%D0%B0)**. Нужно указать **2 или 3** параметра, которые разделяются пробелом. Подставьте **имя_проекта** (без пробелов) и свой **путь_к_каталогу_с_проектами**. Если вы не собираетесь использовать полный набор данных рельефа, то **путь_к_каталогу_с_данными_рельефа** можно не указывать.
        ```
        ./docker_run.sh имя_проекта путь_к_каталогу_с_проектами путь_к_каталогу_с_данными_рельефа
        ```
        Если каталог с проектами не существует то он будет создан.
        
   или
   * Непосредственный запуск docker. Подставьте **имя_проекта** (без пробелов) и свои пути к каталогам. Если вы не используете **путь_к_каталогу_с_данными_рельефа** то **уберите** из следующей команды строчку `--mount type=bind,source=**путь_к_каталогу_с_данными_рельефа**,target=/mnt/terrain \`

     ```
     docker run -dti --rm -e PROJECT_NAME_EXT=**имя_проекта** -e DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix --name qgis-topo \
...
### 3. Подготовка данных
   ...
   1. Заполнить базу данных Overpass
      ...
      Работа с Overpass сервером, установленным внутри *docker*, является самым надёжным вариантом подготовки данных для QGIS-topo. Однако она требует ручного получения данных OSM и заполнения базы данных, что описано выше. Этот процесс может занимать существенное время. Существует возможность получения данных OSM с внешних серверов Overpass. Подробная настройка такого варианта использования описана в шаге [**4**](https://github.com/xmd5a2/qgis-topo#4-%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B0) (переменные **overpass_instance=external** и **overpass_endpoint_external**).
   2. Получить данные рельефа (опционально)
      ...
      * Использовать фрагментарные данные (к примеру, несколько фрагментов нужной области). Их нужно поместить в каталог **путь_к_каталогу_с_проектами/имя_проекта/input_terrain/**.
        ...
        * [CGIAR-CSI SRTM](http://srtm.csi.cgiar.org/srtmdata/) **(рекомендуется)**
        ...
        ```
        предварительно задав в **config.ini** параметр **bbox** (шаг [**4**](https://github.com/xmd5a2/qgis-topo#4-%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B0)).
  ...
  #### Если вы хотите использовать рельеф то не забудьте задать для опции **generate_terrain** значение **true** в **config.ini** (шаг [**4**](https://github.com/xmd5a2/qgis-topo#4-%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B0).
...
### 4. Настройка
  ...
  Обычно требуется менять только переменную **bbox**. Но если вы хотите использовать рельеф (шаг [**3.2**](https://github.com/xmd5a2/qgis-topo#3-%D0%BF%D0%BE%D0%B4%D0%B3%D0%BE%D1%82%D0%BE%D0%B2%D0%BA%D0%B0-%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85)) то нужно для опции **generate_terrain** установить значение **true**.
  
   * **project_name**: имя каталога проекта. Не должно содержать пробелов. Задаётся в шаге [**2**](https://github.com/xmd5a2/qgis-topo#2-%D0%B8%D0%BD%D0%B8%D1%86%D0%B8%D0%B0%D0%BB%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D1%8F) при запуске *docker* контейнера. Менять не требуется.
   ...
   * **array_queries**: список запросов к Overpass, которые будут выполнены в шаге [**5**](https://github.com/xmd5a2/qgis-topo#5-%D0%BF%D0%BE%D0%BB%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5-%D0%B8-%D0%BE%D0%B1%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D0%BA%D0%B0-%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85-osm-%D0%BE%D0%B1%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D0%BA%D0%B0-%D1%80%D0%B5%D0%BB%D1%8C%D0%B5%D1%84%D0%B0). По умолчанию указаны все возможные запросы, но вы можете сократить этот список по своему усмотрению. Обратите внимание что некоторые запросы зависят от других. Эти зависимости описаны в **config.ini (Query dependencies)**. Обычно не требуется менять этот список.
