...
### 3. Подготовка данных
   ...
   2. Получить данные рельефа (опционально)
      ...
      * Использовать полный набор тайлов для всего мира. Поддерживаются только тайлы размером 1х1 градус. Они должны иметь имена, соответствующие общепринятым правилам наименования SRTM. Например: *N51E005.tif*. Поддерживаются форматы GeoTIFF (tif) и HGT (hgt), а также zip архивы с файлами в этих форматах (один архив на файл). Данные должны находиться в каталоге с данными рельефа, который был задан в шаге **2.1.** при запуске docker контейнера.
      ...
      или
      * Использовать иные данные, в частности фрагменты данных, указанных выше. Их нужно поместить в каталог **путь_к_каталогу_с_проектами/имя_проекта/input_terrain/** и изменить значение переменной **get_terrain_tiles** на **false** в файле **config.ini** (шаг **4**).
      
        Для облегчения поиска иных данных возможно получить список необходимых тайлов для текущего проекта:
        ```
        ...
        ```
        предварительно задав в **config.ini** параметр **bbox** (шаг **4.1**).

        Некоторые из подходящих источников данных рельефа:
        * [CGIAR-CSI SRTM](http://srtm.csi.cgiar.org/srtmdata/) **(рекомендуется)**

### 4. Настройка **config.ini**
  Это главный конфигурационный файл проекта. Он находится по **путь_к_каталогу_с_проектами/qgistopo-config/config.ini**. В нём задаются параметры, которые используются в шаге **5. Получение и обработка данных OSM, обработка рельефа**.
