...
if [[ ! -f /.dockerenv ]] ; then
	echo -e "\033[91mThis script is not meant to run outside the docker container. Stopping.\033[0m"
	make_error_flag
	exit 1
fi
if [[ -f $config_dir/config.ini ]] ; then
	...
	echo -e "\033[91mconfig.ini not found. Check project installation integrity. Stopping.\033[0m"
	make_error_flag
	exit 1
fi
...
if [[ ! -d $osm_data_dir ]] ; then
	echo -e "\033[91mosm_data_dir in project_dir does not exist. Stopping.\033[0m"
	make_error_flag
	exit 1
fi
	...
	if [[ $osm_data_is_present == "true" ]] ; then
		...
		if [[ $overpass_endpoint_docker_use_bbox == true ]] ; then
			...
			if [[ $(wc -c <"$osm_tmp_dir/input.osm.bz2") -le 400 ]] ; then
				echo -e "\033[91mError. Cropped extract is empty. Check that bbox parameter matches the OSM data area or turn off 'overpass_endpoint_docker_use_bbox' option.\033[0m"
				make_error_flag 1
				exit 1
			fi
		else
		...
		echo -e "\033[91mError. No OSM data files found in $osm_data_dir.\033[0m"
		make_error_flag
		exit 1
	fi
}
